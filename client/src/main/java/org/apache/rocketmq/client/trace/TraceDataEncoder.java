...
public class TraceDataEncoder {
    ...
    public static List<TraceContext> decoderFromTraceDataString(String traceData) {
        ...
        for (String context : contextList) {
            try {
                decodeRecord(context, resList);
            } catch (IllegalArgumentException | IndexOutOfBoundsException ignored) {
                // One malformed trace must not hide later records in the same batch.
            }
        }
        return resList;
    }

    private static void decodeRecord(String context, List<TraceContext> resList) {
        String[] line = context.split(String.valueOf(TraceConstants.CONTENT_SPLITOR), -1);
        if (line.length >= 12 && line[0].equals(TraceType.Pub.name())) {
